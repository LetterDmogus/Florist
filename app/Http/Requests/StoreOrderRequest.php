...
class StoreOrderRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->input('shipping_type') === 'delivery' && ! $this->filled('delivery_mode')) {
            $this->merge([
                'delivery_mode' => 'new',
            ]);
        }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($this->input('shipping_type') === 'delivery') {
                $deliveryMode = $this->input('delivery_mode', 'new');
            ...
            foreach ($details as $index => $detail) {
                ...
                if ($itemType === 'bouquet') {
                    if ($mode === 'custom') {
                        if (! $this->user()?->can('input custom bouquet')) {
                            $validator->errors()->add("details.{$index}.mode", 'Anda tidak memiliki izin untuk membuat custom bouquet.');
                        }
