...
<template>
    <AppLayout title="Kasir Florist">
        <Head title="Kasir Florist" />

        <div class="space-y-6 max-w-[1600px] mx-auto pb-20">
            <!-- Header -->
            <div class="rounded-3xl border-2 border-pink-200 bg-gradient-to-r from-pink-100/70 via-white to-pink-50 p-6 shadow-sm">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        <div class="p-3 bg-pink-600 rounded-2xl text-white shadow-lg shadow-pink-200">
                            <ShoppingCart class="w-6 h-6" />
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-widest text-pink-600">Florist POS</p>
                            <h1 class="text-2xl font-bold text-pink-950">Buat Pesanan Baru</h1>
                        </div>
                    </div>
            ...
            <div class="grid gap-6 lg:grid-cols-12 items-start">
                                ...
                                <div v-for="unit in bouquetUnits.data" :key="unit.id" class="group relative bg-white border-2 border-pink-100 rounded-3xl p-3 hover:border-pink-400 hover:shadow-xl hover:shadow-pink-100/50 transition-all overflow-hidden">
                                    <div class="aspect-square rounded-2xl bg-pink-50 overflow-hidden mb-3 relative">
                                        ...
                                        <div class="absolute inset-0 bg-pink-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-[2px]">
                                            ...
                                            <button @click="addCatalogItem(unit)" class="p-3 bg-pink-600 rounded-full text-white hover:scale-110 transition-transform shadow-lg" title="Tambah ke Keranjang">
                            ...
                            <div v-if="inventoryItems.links && inventoryItems.links.length > 3" class="flex items-center justify-center gap-1 border-t-2 border-blue-50 pt-4">
                                <template v-for="(link, k) in inventoryItems.links" :key="k">
                                    <button
                                        ...
                                        :class="['px-3 py-1 text-xs font-bold rounded-lg transition-all border-2', link.active ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-blue-100 text-blue-400 hover:border-blue-300']"
                                        v-html="link.label"
                                    />
                                </template>
                            </div>
                ...
                <div class="lg:col-span-5 space-y-6">
                    <section class="bg-white rounded-3xl border-2 border-pink-200 p-6 shadow-xl shadow-pink-100/20">
                        ...
                        <!-- Cart List -->
                        <div class="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2 mb-6">
                            ...
                            <div v-for="item in cartItems" :key="item.cart_id" class="group bg-pink-50/30 border-2 border-pink-100 rounded-3xl p-4 transition-all hover:border-pink-300 hover:bg-white">
                                ...
                                <!-- Greeting & Sender & Money -->
                                <div v-if="item.item_type === 'bouquet'" class="mt-4 grid grid-cols-1 gap-3">
                                    <div class="grid grid-cols-2 gap-3">
                                        ...
                                        <div class="relative">
                                            <input v-model="item.money_amount" type="number" placeholder="Money Bucket (Rp)" class="w-full pl-8 py-2 text-[11px] rounded-xl border-2 border-pink-100 bg-white focus:ring-pink-300 transition-all">
                                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-[11px] font-bold text-pink-300">Rp</span>
                                        </div>
                                    </div>
                                    <div class="relative">
                                        <textarea v-model="item.greeting_card" placeholder="Kartu Ucapan..." rows="2" class="w-full pl-8 py-2 text-[11px] rounded-xl border-2 border-pink-100 bg-white focus:ring-pink-300 transition-all"></textarea>
                                        <MessageSquare class="absolute left-2.5 top-3 w-3.5 h-3.5 text-pink-300" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-pink-950 rounded-3xl p-6 text-white shadow-xl shadow-pink-950/20">
                            ...
                            <BaseButton variant="primary" class="w-full py-4 rounded-2xl bg-pink-600 hover:bg-pink-500 shadow-lg shadow-pink-950/50 text-white border-none font-black tracking-widest transition-all active:scale-95" :disabled="form.processing || cartItems.length === 0" @click="submitOrder">
                                {{ form.processing ? 'SEDANG MEMPROSES...' : 'CHECKOUT ORDER' }}
                            </BaseButton>
                        </div>
                    </section>
                </div>
